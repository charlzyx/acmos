...
export interface VisionSidecarOptions {
  ...
  /** 配置指定的视觉模型，独立于原请求 provider。 */
  visionTarget: ResolvedTarget;
  messages: CcMessage[];
  maxTokens: number;
...
/**
 * 当目标模型不支持图片时，使用视觉 sidecar 生成描述并替换全部图片。
 * sidecar 失败必须保留原始请求，避免辅助能力影响正常请求的可用性。
 */
export async function maybeUseVisionSidecar(opts: VisionSidecarOptions): Promise<VisionSidecarResult> {
  const { target, visionTarget: visualTarget, messages, maxTokens, logger, globalProxy, signal, descriptions } = opts;
  if (target.meta.vision === true) return { messages, sidecarUsed: false };
  if (visualTarget.meta.vision !== true) {
    logger.warn('视觉 sidecar 模型不支持图片，跳过转写', {
  ...
  try {
    ...
    for (const image of images) {
      ...
      if (!description) {
        ...
        const ccStream =
          visualTarget.wire === 'resp' ? translateResponsesSseToCc(response.body) : response.body;
    ...
    logger.warn('识图 sidecar 失败，保留原始图片', {
